...
- 2024/08/18:  1.6 (entries with inactive status in PAT03 ignored)
...
( SELECT
  ...
  ( SELECT
    ...
    FROM
      ...
      ( SELECT ADDON_ID COMP, ADDON_REL RELEASE, SHORT_TEXT, IMPLE_DATE, IMPLE_TIME, DELIV_DATE, DELIV_TIME, TO_REL, STATUS FROM PAT03 UNION ALL
        SELECT COMPONENT COMP, COMP_REL RELEASE, SHORT_TEXT, IMPLE_DATE, IMPLE_TIME, DELIV_DATE, DELIV_TIME, TO_REL, STATUS FROM PAT03
      ) P ON
    ...
    WHERE
      ...
      ( BI.EXCLUDE_SUB_COMPONENTS = ' ' OR UPPER(SUBSTR(P.SHORT_TEXT, 1, 10)) NOT IN ('SUB COMPON', 'SUBCOMPONE') ) AND
      P.STATUS = 'I'
  )
  WHERE
    ( SKIP_PREVIOUS_RELEASES = ' ' OR LINE_NO_2 = 1  )
