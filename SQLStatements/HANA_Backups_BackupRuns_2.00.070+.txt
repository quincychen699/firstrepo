...
- HANA_Backups_BackupRuns_2.00.070+
...
- M_BACKUP_CATALOG_FILES.COMPRESSED_SIZE available with SAP HANA >= 2.00.060
- M_BACKUP_CATALOG.RETAINED available with SAP HANA >= 2.00.070
- In case of backups with multiple streams the total backup runtime is split across the different streams based on their backup volume, so shorter 
  durations than actually needed will be displayed in RUNTIME_MIN in case no aggregation by BACKUP_ID is used.
...
- 2023/08/26:  2.2 (dedicated 2.00.070+ version including RETAINED)
- 2024/07/08:  2.3 (COMPRESSED_SIZE included)
...
- RETAINED
  ...
  Indication if data backup is retained

  'TRUE'          --> Display data backups that are retained for longer than the backup period
  'FALSE'         --> Display data backups that are not retained for longer than the backup period
  '%'             --> No restriction related to data backup retention
...
- R:                'X' if data backup is retained, otherwise ' ' ('a' for 'any')
- RUNTIME_MIN:      Runtime of backups (in minutes)
- BACKUP_SIZE_MB:   Backup size (MB)
- COMP_SIZE_MB:     Compressed backup size (MB)
...
*/
  ...
  MAP(RETAINED, 'TRUE', 'X', 'FALSE', ' ', SUBSTR(RETAINED, 1, 1)) R,
  IFNULL(LPAD(TO_DECIMAL(RUNTIME_H * 60, 10, 2), 11), '') RUNTIME_MIN,
  LPAD(TO_DECIMAL(BACKUP_SIZE_MB, 10, 2), 14) BACKUP_SIZE_MB,
  LPAD(TO_DECIMAL(COMP_SIZE_MB, 10, 2), 14) COMP_SIZE_MB,
  IFNULL(LPAD(TO_DECIMAL(MAP(RUNTIME_H, 0, 0, COMP_SIZE_MB / RUNTIME_H / 3600), 10, 2), 8), '') MB_PER_S,
...
( SELECT
    ...
    END BACKUP_SIZE_MB,
    CASE AGGREGATION_TYPE
      WHEN 'SUM' THEN SUM_COMP_SIZE_MB
      WHEN 'AVG' THEN MAP(NUM_BACKUP_RUNS, 0, 0, SUM_COMP_SIZE_MB / NUM_BACKUP_RUNS)
      WHEN 'MAX' THEN MAX_COMP_SIZE_MB
    END COMP_SIZE_MB,
    MAX_START_TIME,
    MESSAGE,
    CONF_INCLUDED,
  ...
  ( SELECT
      ...
      CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'RETAINED')         != 0 THEN IFNULL(B.RETAINED, '')                          ELSE MAP(BI.RETAINED, '%', 'any', BI.RETAINED)                 END RETAINED,
      ...
      IFNULL(SUM(BF.COMPRESSED_SIZE / 1024 / 1024 ), 0) SUM_COMP_SIZE_MB,
      IFNULL(MAX(BF.COMPRESSED_SIZE / 1024 / 1024 ), 0) MAX_COMP_SIZE_MB,
    ...
    ( SELECT
      ...
      ( SELECT                                                                  /* Modification section */
          ...
          '%' HOST,
          '%' SERVICE_NAME,
          'DATA_BACKUP' BACKUP_TYPE,                             /* e.g. 'log backup', 'complete data backup', 'incremental data backup', 'differential data backup', 'data snapshot',
          ...
          'TIME' AGGREGATE_BY,        /* HOST, SERVICE, TIME, BACKUP_ID, BACKUP_TYPE, BACKUP_DATA_TYPE, STATE, MESSAGE, CONF_INCLUDED, RETAINED or comma separated list, NONE for no aggregation */
      ...
      ( SELECT
          ...
          SOURCE_TYPE_NAME,
          BACKUP_SIZE,
          IFNULL(COMPRESSED_SIZE, BACKUP_SIZE) COMPRESSED_SIZE,
      ...
      WHERE
        ...
        IFNULL(BF.CONF_INCLUDED, 'FALSE') LIKE BI.CONF_INCLUDED AND
        IFNULL(B.RETAINED, '') LIKE BI.RETAINED
    GROUP BY
      CASE 
        WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'TIME') != 0 THEN 
          CASE 
      ...
      CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'RETAINED')         != 0 THEN IFNULL(B.RETAINED, '')                          ELSE MAP(BI.RETAINED, '%', 'any', BI.RETAINED)                 END,
      BI.MIN_BACKUP_TIME_S,
      BI.AGGREGATION_TYPE,
      BI.AGGREGATE_BY,
      BI.ORDER_BY
