...
HANA_GarbageCollection_Blockers_History_2.00.060+
...
- 2022/05/24:  1.5 (dedicated 2.00.060+ version including SITE_ID)
- 2024/03/28:  1.6 (ORDER_BY included)
...
- ORDER_BY
  ...
  'SNAPSHOT_TIME' --> Sorting by snapshot time
...
*/
  ...
  ISSUE_TYPE,
  IFNULL(TO_VARCHAR(MIN(S_TIME), 'YYYY/MM/DD HH24:MI:SS'), '') START_TIME,
...
( SELECT
  ...
  ( SELECT
    ...
    ( SELECT                     /* Modification section */
        ...
        'DURATION' ORDER_BY           /* SNAPSHOT_TIME, START_TIME, DURATION */
      FROM
        DUMMY
    )
  ) BI,
  ...
  WHERE
    ...
    ( BI.SITE_ID = -1 OR ( BI.SITE_ID = 0 AND G.SITE_ID IN (-1, 0) ) OR G.SITE_ID = BI.SITE_ID ) AND
...
GROUP BY
  ...
  SQL_TEXT_LENGTH
ORDER BY
  MAP(ORDER_BY, 'SNAPSHOT_TIME', SERVER_TIMESTAMP, 'START_TIME', START_TIME, 'DURATION', DURATION_S) DESC,
