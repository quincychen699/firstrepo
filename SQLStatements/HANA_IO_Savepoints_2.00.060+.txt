...
- 2022/05/26:  2.5 (dedicated 2.00.060+ version including SITE_ID for data source HISTORY)
- 2022/10/05:  2.5 (critical phase details added)
- 2023/02/16:  2.6 (FLUSH_S phase information added)
...
- MIN_WAIT_FOR_FLUSH_PHASE_DURATION_S

  Only show savepoints with a waitForFlush phase that exceeds the configured threshold (in seconds)

  10               --> Display savepoints with a waitForFlush phase of at least 10 seconds
  -1               --> Display all savepoints
...
- FLUSH_S:               waitForFlush phase duration (s)
...
*/
  ...
  LPAD(TO_DECIMAL(WAIT_FOR_FLUSH_PHASE_S, 12, 2), 10) FLUSH_S,
...
( SELECT
  ...
  ( SELECT
      CASE 
        ...
        ELSE 'any' 
      END START_TIME,
      CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'SITE_ID')    != 0 THEN TO_VARCHAR(SP.SITE_ID) ELSE MAP(BI.SITE_ID,     -1, 'any', TO_VARCHAR(BI.SITE_ID)) END SITE_ID,
      ...
      MAP(BI.AGGREGATION_TYPE, 'MAX', MAX(SP.WAIT_FOR_FLUSH_DURATION), 'SUM', SUM(SP.WAIT_FOR_FLUSH_DURATION), 'AVG', AVG(SP.WAIT_FOR_FLUSH_DURATION)) / 1000000 WAIT_FOR_FLUSH_PHASE_S,
    ...
    ( SELECT
      ...
      ( SELECT                                                         /* Modification section */
          ...
          -1 MIN_WAIT_FOR_FLUSH_PHASE_DURATION_S,
          ...
          'NONE' AGGREGATE_BY,         /* TIME, SITE_ID, HOST, PORT, INITIATION, PURPOSE or comma separated combinations, NONE for no aggregation */
    ...
    ( SELECT
        ...
        BLOCKING_PHASE_DURATION - CRITICAL_PHASE_WAIT_TIME - CRITICAL_PHASE_DURATION WAIT_FOR_FLUSH_DURATION,
        CRITICAL_PHASE_DURATION,
        BLOCKING_PHASE_DURATION,
        TOTAL_SIZE,
    ...
    WHERE
      ( BI.SITE_ID = -1 OR ( BI.SITE_ID = 0 AND SP.SITE_ID IN (-1, 0) ) OR SP.SITE_ID = BI.SITE_ID ) AND
      ...
      ( BI.MIN_WAIT_FOR_FLUSH_PHASE_DURATION_S = -1 OR SP.WAIT_FOR_FLUSH_DURATION / 1000000 >= BI.MIN_WAIT_FOR_FLUSH_PHASE_DURATION_S ) AND
    ...
    GROUP BY
      CASE 
        ...
        ELSE 'any' 
      END,
      CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'SITE_ID')    != 0 THEN TO_VARCHAR(SP.SITE_ID) ELSE MAP(BI.SITE_ID,     -1, 'any', TO_VARCHAR(BI.SITE_ID)) END,
