...
- HANA_Security_GrantedRolesAndPrivileges_2.00.000+
...
- ROLE_SCHEMA_NAME available with SAP HANA >= 2.0 SPS 00
...
- 2022/05/06:  1.6 (redesign based on SQL hierarchies)
- 2023/10/14:  1.7 (grantee type included in output, column T)
- 2024/02/25:  1.8 (ONLY_INTERNAL_PRIVILEGES included)
...
- ONLY_INTERNAL_PRIVILEGES

  Possibility to restrict the result to internal privileges (that should not be used by external users)

  'X'             --> Only display internal privileges
  ' '             --> No restriction related to internal privileges
...
( SELECT GRANTEE, GRANTEE_TYPE, IFNULL(GRANTEE_SCHEMA_NAME, '') GRANTEE_SCHEMA_NAME, ROLE_NAME GRANTED, IFNULL(ROLE_SCHEMA_NAME, '') ROLE_SCHEMA_NAME, '' OBJECT_TYPE, '' SCHEMA_NAME, '' OBJECT_NAME, '' COLUMN_NAME, 'R' NODE_TYPE FROM GRANTED_ROLES
  UNION ALL
  ...
  UNION ALL
  SELECT GRANTEE, GRANTEE_TYPE, IFNULL(GRANTEE_SCHEMA_NAME, ''), PRIVILEGE, '', IFNULL(OBJECT_TYPE, '') OBJECT_TYPE, IFNULL(SCHEMA_NAME, '') SCHEMA_NAME, IFNULL(OBJECT_NAME, '') OBJECT_NAME, 
    IFNULL(COLUMN_NAME, '') COLUMN_NAME, 'P' FROM GRANTED_PRIVILEGES
),
ROLES_HIERARCHY AS
( SELECT 
    * 
...
( SELECT 
    *,
    ...
    RTRIM(LTRIM(SCHEMA_NAME || '.' || OBJECT_NAME || '.' || COLUMN_NAME, '.'), '.') PRIVILEGE_DETAILS
  FROM 
...
WHERE
  ...
  P.FINAL_GRANTEE_TYPE LIKE SUBSTR(BI.GRANTEE_TYPE, 1, 1) AND
  ...
  ( BI.ONLY_INTERNAL_PRIVILEGES = ' ' OR
    P.NODE_TYPE = 'P' AND P.GRANTED NOT IN ( SELECT NAME FROM PRIVILEGES )
  ) AND
  P.HIERARCHY_LEVEL > 1
ORDER BY
  P.FINAL_GRANTEE,
