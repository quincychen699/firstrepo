...
- Unlike other monitoring views, M_SERVICE_MEMORY also contains information about secondary system replication sites, but no SITE_ID column.
- Value in output column NON_FREEABLE_GB may be too small with SAP HANA <= 2.00.059.09, <= 2.00.067.02 and <= 2.00.071 due to bug 305893
- FAST_RESTART_MEMORY_MAPPED_SIZE available with SAP HANA >= 2.00.055
...
- 2023/08/08:  1.9 (SERVICE_NAME removed, ADDR_USED_PCT included)
- 2024/01/31:  2.0 (HEAP_UNUSED_GB included, dedicated 2.00.055+ version including FAST_RESTART_MEMORY_MAPPED_SIZE)
- 2024/07/25:  2.1 (MIN_SEGMENT_SIZE included)
...
- HOST:             Host name
- PORT:             Port
- ALLOC_LIM_GB:     Process allocation limit (GB)
- EFF_ALLOC_LIM_GB: Effective process allocation limit (GB)
...
- HEAP_UNUSED_GB:   Heap memory allocated but unused (GB)
- SHARED_GB:        Allocated shared memory (GB), can be treated as used memory as reduction of shared memory is not possible online
- PERSMEM_GB:       Persistent memory / fast restart option memory (GB)
...
- ADDR_USED_PCT:    Used address space (%)
- MIN_SEG_SIZE_MB:  Minimum se<gment size (MB)
...
*/
  ...
  HOST,
  LPAD(PORT, 5) PORT,
  LPAD(TO_DECIMAL(ALLOC_LIMIT_GB, 10, 2), 12) ALLOC_LIM_GB,
  LPAD(TO_DECIMAL(EFF_ALLOC_LIMIT_GB, 10, 2), 16) EFF_ALLOC_LIM_GB,
  ...
  LPAD(TO_DECIMAL(GREATEST(0, HEAP_ALLOC_GB - HEAP_USED_GB), 10, 2), 14) HEAP_UNUSED_GB,
  LPAD(TO_DECIMAL(SHARED_ALLOC_GB, 10, 2), 9) SHARED_GB,
  LPAD(TO_DECIMAL(PERSMEM_GB, 10, 2), 10) PERSMEM_GB,
  LPAD(TO_DECIMAL(GREATEST(0, HEAP_USED_GB - FREEABLE_GB), 10, 2), 15) NON_FREEABLE_GB,
  ...
  LPAD(TO_DECIMAL(MAP(ADDR_TOT_GB, 0, 0, ADDR_USED_GB / ADDR_TOT_GB * 100), 10, 2), 13) ADDR_USED_PCT,
  LPAD(TO_DECIMAL(MIN_SEG_SIZE_MB, 10, 2), 15) MIN_SEG_SIZE_MB
FROM
( SELECT
    SNAPSHOT_TIME,
    ...
    MAX(VIRTUAL_ADDRESS_SPACE_USED_SIZE) / 1024 / 1024 / 1024 ADDR_USED_GB,
    AVG(VIRTUAL_ADDRESS_SPACE_TOTAL_SIZE) / 1024 / 1024 / 1024 ADDR_TOT_GB,
    AVG(MIN_SEGMENT_SIZE) / 1024 / 1024 MIN_SEG_SIZE_MB,
  ...
  ( SELECT
      ...
      AVG(M.MIN_SEGMENT_SIZE) MIN_SEGMENT_SIZE,
    ...
    ( SELECT
      ...
      ( SELECT                       /* Modification section */
          ...
          'TIME' AGGREGATE_BY,        /* HOST, PORT, TIME or comma separated combinations, NONE for no aggregation */
    ...
    ( SELECT
        ...
        MIN_SEGMENT_SIZE,
        FAST_RESTART_MEMORY_MAPPED_SIZE + PERSISTENT_MEMORY_MAPPED_SIZE PERSISTENT_MEMORY_SIZE
      FROM
        M_SERVICE_MEMORY S
      UNION ALL
      SELECT
