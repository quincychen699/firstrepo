...
- Overview of Smart Data Access (SDA) and Smart Data Integration (SDI) sources
...
- 2024/04/21:  1.2 (LOCATION and TYPE added)
...
- LOCATION
  ...
  'indexserver'   --> Location service name indexserver
  '%'             --> No restriction related to location service name
...
- LOCATION:           Service name
- TYPE:               SDA for smart data access, SDI for smart data integration
...
*/
  ...
  MAP(LINE_NO, 1, LOCATION, '') LOCATION,
  MAP(LINE_NO, 1, MAP(LOCATION, 'indexserver', 'SDA', 'scriptserver', 'SDA', 'SDI'), '') TYPE,
...
( SELECT
  ...
  ( SELECT
    ...
    WHERE
      ...
      S.LOCATION LIKE BI.LOCATION AND
      MAP(S.LOCATION, 'indexserver', 'SDA', 'scriptserver', 'SDA', 'SDI') LIKE BI.TYPE AND
