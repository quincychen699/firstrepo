...
- HANA_Tables_ColumnStore_Merges_2.00.060+
...
- 2023/01/10:  2.4 (MIN_BLOCKING_LOCK_WAIT_TIME_S filter included)
- 2023/08/19:  2.5 (MIN_RESOURCE_WAIT_TIME_S filter included)
...
- MIN_RESOURCE_WAIT_TIME_S

  Threshold for minimum resource wait time (s), i.e. waiting for availability of tokens

  600             --> Only display table optimizations with at least 600 seconds of resource wait time
  -1              --> No restriction related to resource wait time
...
- ONLY_FAILED_MERGES
  ...
  'X'             --> Only show merges that were reported with an error
  ' '             --> Show all merges
...
( SELECT
    ...
    CASE 
      ...
      ELSE 'any' 
    END END_TIME,
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'SITE_ID')    != 0 THEN TO_VARCHAR(M.SITE_ID)                           ELSE MAP(BI.SITE_ID,      -1, 'any', TO_VARCHAR(BI.SITE_ID))   END SITE_ID,
  ...
  ( SELECT
    ...
    ( SELECT                                                /* Modification section */
        ...
        'NONE' AGGREGATE_BY,           /* TIME, SITE_ID, HOST, PORT, SCHEMA, TABLE, TYPE, MOTIVATION, ERROR and comma-separated combinations, NONE for no aggregation */
  ...
  WHERE
    ...
    ( BI.MIN_RESOURCE_WAIT_TIME_S = -1 OR M.RW_S >= BI.MIN_RESOURCE_WAIT_TIME_S ) AND
  ...
  GROUP BY
    ...
    CASE 
      ...
      ELSE 'any' 
    END,
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'SITE_ID')    != 0 THEN TO_VARCHAR(M.SITE_ID)                           ELSE MAP(BI.SITE_ID,      -1, 'any', TO_VARCHAR(BI.SITE_ID))   END,
