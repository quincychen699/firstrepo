...
- HANA_Tables_ColumnStore_TableSize_2.00.043+
...
- 2023/11/29:  2.6 (MIN_TOTAL_NSE_SIZE_GB included)
...
- MIN_TOTAL_NSE_SIZE_GB

  Lower limit for total size in NSE buffer (GB)

  10              --> Only display tables occupying at least 10 GB in the NSE buffer
  -1              --> No restriction related to NSE buffer size
...
( SELECT
  ...
  ( SELECT                                                      /* Modification section */
      ...
      -1 MIN_TOTAL_NSE_SIZE_GB,
      -1 MAX_UNUSED_RETENTION_PERIOD_S,
      '%' IS_LOG_DELTA,
      ' ' ONLY_HISTORY_TABLES,
      'TABLE' OBJECT_LEVEL,
      'CURRENT' DATA_SOURCE,       /* CURRENT, HISTORY */
      'NONE' AGGREGATE_BY,         /* TIME, HOST, PORT, SCHEMA, TABLE, RETENTION or comma separated combinations, NONE for no aggregation */
...
WHERE
  ...
  ( MIN_TOTAL_SIZE_GB = -1 OR TOTAL_BYTE / 1024 / 1024 / 1024 >= MIN_TOTAL_SIZE_GB ) AND
  ( MIN_TOTAL_NSE_SIZE_GB = -1 OR PAGED_BYTE / 1024 / 1024 / 1024 >= MIN_TOTAL_NSE_SIZE_GB )
ORDER BY
  ROW_NUM