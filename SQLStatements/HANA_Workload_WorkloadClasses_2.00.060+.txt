...
- HANA_Workload_WorkloadClasses_2.00.060+
...
- STATEMENT_TIMOUT, IDLE_CURSOR_LIFETIME and WRITE_TRANSACTION_LIFETIME available with SAP HANA >= 2.00.040
- WORKLOAD_CLASSES.PARENT_WORKLOAD_CLASS_NAME available with SAP HANA >= 2.00.060
- ADMISSION_CONTROL_REJECT_CPU_THRESHOLD, ADMISSION_CONTROL_REJECT_MEMORY_THRESHOLD, ADMISSION_CONTROL_QUEUE_CPU_THRESHOLD
  and ADMISSION_CONTROL_QUEUE_MEMORY_THRESHOLD available with SAP HANA >= 2.00.060
...
- 2024/06/08:  1.5 (dedicated 2.00.060+ version including PARENT_WORKLOAD_CLASS_NAME)
- 2024/06/21:  1.6 (admission control columns included)
- 2024/09/11:  1.7 (IDLE_CURS_MIN und UPD_TRANS_MIN included)
...
- PARENT_WORKLOAD_CLASS

  Parent workload class name

  'MY_PWORKLOAD'  --> Restrict output to entries related to parent workload class MY_PWORKLOAD
  '%'             --> No restriction
...
- UPD_TRANS_MIN:    Update (write) transaction lifetime (min)
- IDLE_CURS_MIN:    Idle cursor lifetime (min)
- REJECT_CPU_PCT:   Admission control reject CPU threshold (%)
- REJECT_MEM_PCT:   Admission control reject memory threshold (%)
- QUEUE_CPU_PCT:    Admission control queue CPU threshold (%)
- QUEUE_MEM_PCT:    Admission control queue memory threshold (%)
...
*/
  ...
  WC.PARENT_WORKLOAD_CLASS_NAME PARENT_CLASS,
  MAP(WC.IS_ENABLED, 'TRUE', 'X', '') E,
  ...
  IFNULL(LPAD(WC.WRITE_TRANSACTION_LIFETIME, 13), '') UPD_TRANS_MIN,
  IFNULL(LPAD(WC.IDLE_CURSOR_LIFETIME, 13), '') IDLE_CURS_MIN,
  IFNULL(LPAD(WC.ADMISSION_CONTROL_REJECT_CPU_THRESHOLD, 14), '') REJECT_CPU_PCT,
  IFNULL(LPAD(WC.ADMISSION_CONTROL_REJECT_MEMORY_THRESHOLD, 14), '') REJECT_MEM_PCT,
  IFNULL(LPAD(WC.ADMISSION_CONTROL_QUEUE_CPU_THRESHOLD, 13), '') QUEUE_CPU_PCT,
  IFNULL(LPAD(WC.ADMISSION_CONTROL_QUEUE_MEMORY_THRESHOLD, 13), '') QUEUE_MEM_PCT,
...
( SELECT                /* Modification section */
  ...
  WORKLOAD_CLASSES WC ON
    WC.WORKLOAD_CLASS_NAME LIKE BI.WORKLOAD_CLASS AND
    WC.PARENT_WORKLOAD_CLASS_NAME LIKE BI.PARENT_WORKLOAD_CLASS LEFT OUTER JOIN
